...
1. Verwende `across()`, um die mittlere Anzahl an Schlafzimmern (`schlafzimmer`) und Badezimmern (`badezimmer`) zu berechnen. 

```{r eval = FALSE, echo = TRUE}
kc_house %>%
  summarise(across(.cols = c(XX, XX), .fns = XX))
```

```{r}
kc_house %>%
  summarise(across(.cols = c(schlafzimmer, badezimmer), .fns = mean))
```

1. Verwende `across()` und `starts_with()`, um den mittlere grösse aller Quadratmeterzahlen zu bestimmen. 

```{r eval = FALSE, echo = TRUE}
kc_house %>%
  summarise(across(.cols = starts_with(XX), .fns = XX))
```

```{r}
kc_house %>%
  summarise(across(.cols = starts_with("qf_"), .fns = mean))
```
